...
class FaceReconModel(BaseModel):
    ...
    def forward(self):
        ...
        print(self.pred_coeffs_dict['angle'])
        print(self.pred_coeffs_dict['trans'])
    ...
    def compute_visuals(self):
        with torch.no_grad():
            ...
            output_vis_depth_raw = self.pred_depth.detach().cpu()
            output_vis_depth_raw *= -1
            output_vis_depth_raw[self.pred_mask == 1] -= output_vis_depth_raw[self.pred_mask == 1].min()
            output_vis_depth_raw[self.pred_mask == 1] /= output_vis_depth_raw[self.pred_mask == 1].max()
            output_vis_depth = 255. * output_vis_depth_raw.expand(-1, 3, -1, -1).permute(0, 2, 3, 1).numpy()
#             print(output_vis_depth.min(), output_vis_depth.max())
            ...
            if self.gt_lm is not None:
                ...
                output_vis_numpy = np.concatenate((input_img_numpy, 
                                    output_vis_depth, output_vis_numpy_raw, output_vis_numpy), axis=-2)
            else:
                output_vis_numpy = np.concatenate((input_img_numpy, 
